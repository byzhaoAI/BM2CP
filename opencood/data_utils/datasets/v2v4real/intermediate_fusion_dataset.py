...
from opencood.utils.transformation_utils_v2v4real import x1_to_x2, dist_two_pose
from opencood.utils import box_utils_v2v4real as box_utils
from opencood.utils.box_utils_v2v4real import boxes_to_corners2d


class IntermediateFusionDataset(basedataset.BaseDataset):
    def __init__(self, params, visualize, train=True, isSim=False):
        super(IntermediateFusionDataset, self).__init__(params, visualize, train, isSim)
        self.pre_processor = build_preprocessor(params['preprocess'], train)
        self.post_processor = post_processor.build_postprocessor(params['postprocess'], dataset='v2v4real', train=train)
    ...
    def __getitem__(self, idx):
        ...
        for i, (cav_id, selected_cav_base) in enumerate(base_data_dict.items()):
            ...
            if self.visualize:
                ...
                projected_lidar = np.r_[projected_lidar.T, [np.ones(projected_lidar.shape[0])*i]].T
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        object_bbx_center, object_bbx_mask, object_ids = self.post_processor.generate_object_center_v2v4real([selected_cav_base], transformation_matrix if not self.isSim else ego_pose)
        ...
        # data augmentation
        lidar_np, object_bbx_center, object_bbx_mask = self.augment(lidar_np, object_bbx_center, object_bbx_mask, selected_cav_base['flip'], selected_cav_base['noise_rotation'], selected_cav_base['noise_scale'])
