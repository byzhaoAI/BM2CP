        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    bev_file = os.path.join(cav_path, timestamp + '_bev.png')
                    ...
                    self.scenario_database[i][cav_id][timestamp]['bev'] = bev_file
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            if os.path.exists(cav_content[timestamp_key_delay]['bev']):
                ...
                data[cav_id]['bev_map'] = bev_map.astype(float) / bev_map.max()
            data[cav_id]['folder_name'] = cav_content[timestamp_key_delay]['lidar'].split('/')[-3]
            data[cav_id]['index'] = timestamp_index
            data[cav_id]['cav_id'] = int(cav_id)
            data[cav_id]['frame_id'] = (data[cav_id]['folder_name'], cav_content[timestamp_key_delay]['lidar'].split('/')[-1][:-4])
        return data
