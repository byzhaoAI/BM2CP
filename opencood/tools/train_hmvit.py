...
def main():
    ...
    second_net, pillar_net, mm_net = None, None, None
    fusion_net, decoder, cls_head, reg_head, dir_head = None, None, None, None, None

    if 'second' in model_args and 'model_path' in model_args['second']:
        sec_net_hypes = yaml_utils.load_yaml(model_args['second']['model_hypes'], opt)
        second_net = train_utils.create_model(sec_net_hypes)
        _, second_net = train_utils.load_saved_model(model_args['second']['model_path'], second_net)

        fusion_net = second_net.fusion_net
        decoder = second_net.decoder
        cls_head = second_net.cls_head
        reg_head = second_net.reg_head
        if hasattr(second_net, 'dir_head'):
            dir_head = second_net.dir_head
        
        if model_args['second']['model_freeze']:
            print('load multimodal networks')
            second_net = second_net.second
        else:
            second_net = None
    
    if 'multimodal' in model_args and 'model_path' in model_args['multimodal']:
        mm_net_hypes = yaml_utils.load_yaml(model_args['multimodal']['model_hypes'], opt)
        mm_net = train_utils.create_model(mm_net_hypes)
        _, mm_net = train_utils.load_saved_model(model_args['multimodal']['model_path'], mm_net)

        if 'second' not in model_args:
            fusion_net = mm_net.fusion_net
            decoder = mm_net.decoder
            cls_head = mm_net.cls_head
            reg_head = mm_net.reg_head
            if hasattr(mm_net, 'dir_head'):
                dir_head = mm_net.dir_head
        
        if model_args['multimodal']['model_freeze']:
            print('load multimodal networks')
            mm_net = mm_net.basebone
        else:
            mm_net = None
    
    if 'pillar' in model_args and  'model_path' in model_args['pillar']:
        pillar_net_hypes = yaml_utils.load_yaml(model_args['pillar']['model_hypes'], opt)
        pillar_net = train_utils.create_model(pillar_net_hypes)
        _, pillar_net = train_utils.load_saved_model(model_args['pillar']['model_path'], pillar_net)
        
        if 'second' not in model_args and 'multimodal' not in model_args:
            fusion_net = pillar_net.fusion_net
            decoder = pillar_net.decoder
            cls_head = pillar_net.cls_head
            reg_head = pillar_net.reg_head
            if hasattr(mm_net, 'dir_head'):
                dir_head = mm_net.dir_head

        if model_args['pillar']['model_freeze']:
            print('load pillar networks')
            pillar_net = pillar_net.piilar
        else:
            pillar_net = None
    
    if pillar_net is not None or mm_net is not None:
        model.update_model(fusion_net, decoder, cls_head, reg_head, dir_head, second_net, pillar_net, mm_net)
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            if 'scope' in hypes['name'] or 'how2comm' in hypes['name'] or 'syncnet' in hypes['name']:
                ...
                if 'vqm' in hypes['name'] or 'hetero' in hypes['name']:
                    if opt.random:
                ...
                if 'hetero' in hypes['name']:
                        final_loss = final_loss + output_dict['bfp_loss']
                else:
