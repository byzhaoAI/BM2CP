...
class ImgCamEncode(nn.Module):  # 提取图像特征进行图像编码
    ...
    def get_gt_depth_dist(self, x):  # 对深度维进行onehot，得到每个像素不同深度的概率
        ...
        Returns:
            x: [B*N, D, fH, fW]
        """
        target = False # self.training
        # print(x.max(), x.min())
        # plt.imshow(x[0].cpu().numpy(), cmap='gray')
        # plt.savefig('depth_ori_show.png')
        # plt.close()
        ...
        # print(depth_indices.max(), depth_indices.min())
        # plt.imshow(depth_indices[0].cpu().numpy(), cmap='gray')
        # plt.savefig('depth_show.png')
        # plt.close()
