...
from opencood.models.fecp_v2_modules.base_bev_backbone_resnet import ResNetBEVBackbone
from opencood.models.fecp_v2_modules.attentioncomm import ScaledDotProductAttention, AttenComm
from opencood.models.fecp_v2_modules.sensor_blocks import ImgCamEncode
...
class PointPillarFECPV2(nn.Module):
    ...
    def __init__(self, args):
        super(PointPillarFECPV2, self).__init__()
        # cuda选择
    ...
    def forward(self, data_dict_list):   # loss: 5.91->0.76
        semantic_map = None
        ...
        for origin_data in data_dict_list:
            ...
            if self.multi_scale:
                temporal_data_list.append(batch_dict['spatial_features'])
            else:
                temporal_data_list.append(spatial_features_2d)

            if semantic_map is None:
                semantic_map = self.cls_head(spatial_features_2d)
        ...
        # record_len check
        init_record_len = record_len_list[0]
        # for rlen in record_len_list[1:]:
        #     eval_list = (init_record_len == rlen)
        #     assert False not in eval_list, "Error in record length."
