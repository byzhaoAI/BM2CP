...
class CoVQMF1(nn.Module):   
    def __init__(self, args):
        ...
        if self.use_camera:
            ...
            self.method = 'unproj' if method in img_args and img_args['method'] == 'unproj' else 'lss'
            if self.method == 'unproj':
                self.camera_encoder = CoVQMF3(args)
            else:
                self.camera_encoder = LSS(args)
    ...
    def forward(self, batch_dict, mode=[0,1], training=True):
        ...
        if self.use_camera:
            if self.method == 'unproj':
                camera_feature = self.camera_encoder(batch_dict['image_inputs'])
            else:
                camera_feature = self.camera_encoder(batch_dict)
            # if not self.use_lidar:
            #     camera_feature = self.minmax_norm(camera_feature)
        ...
        if 1 in mode and self.use_camera:
            # modal_features.append(self.camera_proj(camera_feature))
            modal_features.append(camera_feature)
            m_len += 1
        modal_features, rec_loss, svd_loss = self.fusion(modal_features, training=training)
        '''
        camera_feature, rec_loss, svd_loss = self.mask_modality(lidar_feature, camera_feature, training, batch_dict['record_len'])
        return modal_features.unsqueeze(1), 1, rec_loss, svd_loss
