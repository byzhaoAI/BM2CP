...
class PyramidFusion(ResNetBEVBackbone):
    def __init__(self, model_cfg, input_channels=64):
        ...
        self.f2_backward_proj = nn.Conv2d(args['fusion']['num_filters'][0], args['fusion']['num_filters'][0], kernel_size=3, stride=1, padding=1) 
        self.f3_backward_proj = nn.Conv2d(args['fusion']['num_filters'][0], args['fusion']['num_filters'][0], kernel_size=3, stride=1, padding=1) 
    ...
    def forward_collab(self, spatial_features, record_len, affine_matrix, fp = False, align_last = False):
        """
