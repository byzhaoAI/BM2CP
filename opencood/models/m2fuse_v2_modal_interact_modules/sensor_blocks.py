...
from opencood.models.m2fuse_v2_modal_interact_modules.grid_attention import GridAttEncoder
...
class ImgCamEncode(nn.Module):  # 提取图像特征进行图像编码
    def __init__(self, img_size, D, C, Z, chain_channels, downsample, ddiscr, mode, att_args, use_gt_depth=False, depth_supervision=True):
    ...
    def forward(self, x, pc_voxels, record_len):
        ...
        for idx in range(len(record_len)):
            ...
            # (4, 1, 256, 45, 60)
            pc_voxel = self.resizer(self.resize_att(pc_voxel))
