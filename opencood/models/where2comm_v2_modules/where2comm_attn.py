...
from opencood.models.where2comm_v2_modules.where2comm import Communication
from opencood.models.where2comm_v2_modules.grid_attention import GridAttEncoder
...
class MultiModalFusion(nn.Module):
    ...
    def forward(self, pc_feat, img_feat):
        fused_feat = pc_feat + self.grid_att(pc_feat, img_feat)
        thres_map = self.adapt_cls(self.adapt_conv(fused_feat))
        return fused_feat, thres_map
...
class Where2comm(nn.Module):
    def __init__(self, args):
        ...
        if self.multi_scale:
            ...
            self.modal_modules = MultiModalFusion(args['grid_att'], args['in_channels'])
            if self.agg_mode == 'ATTEN':
    ...
    def forward(self, img_feat, x, rm, record_len, pairwise_t_matrix, backbone=None, heads=None):
        """
        Fusion forwarding.
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    ...
                    if self.communication:
                        ...
                        _, communication_masks, communication_rates = self.naive_communication(batch_x, batch_confidence_maps, batch_threshold_map, record_len, pairwise_t_matrix)
            ...
            if self.communication:
                _, communication_masks, communication_rates = self.naive_communication(batch_x, batch_confidence_maps, record_len, pairwise_t_matrix)
            else:
                communication_rates = torch.tensor(0).to(x.device)
