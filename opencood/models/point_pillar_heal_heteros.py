...
class PointPillarHEALHeteros(nn.Module):    
    def __init__(self, args):
        super(PointPillarHEALHeteros, self).__init__()
        # cuda选择
        self.device = args['device']
        self.max_cav = args['max_cav']
        self.cav_range = args['lidar_range']
        self.freeze_backbone = args['freeze_backbone'] if 'freeze_backbone' in args else False
        self.align_with_ego = args['align_with_ego'] if 'align_with_ego' in args else False
        print('freeze_backbone, align_with_ego: ', self.freeze_backbone, self.align_with_ego)
        ...
        for agent_uid in self.agent_types:
            ...
            if modality_num > 1:
                ...
                setattr(self, f"{agent_uid}_fusion", nn.Conv2d(self.agent_types[agent_uid]['fusion_channel']*modality_num, self.agent_types[agent_uid]['fusion_channel'],1,1,0))
    ...
    def forward_collab(self, data_dict, mode=[0,1], training=False, visualization=False):
        ...
        for agent_idx, agent_uid in enumerate(self.agent_types):
            ...
            if len(f) > 1:
                f = eval(f"self.{agent_uid}_fusion")(torch.cat(f, dim=1))
            elif len(f) == 1:
                f = f[0]
            else:
                raise
            ...
            if len(self.agent_types) == 1:
                ...
                if agent_idx > 0:
                    # print(agent_idx, agent_uid, modality_num)
                    new_agent_features.append(torch.cat(select_ego, dim=0))
        ...
        # record_len = torch.ones(record_len.shape).long().to(record_len.device)
