...
class TemporalSampling(nn.Module):
    def __init__(self, feature_dim, frame, decay_rate=0.5, threshold=0.5):
        ...
        for i in range(frame):
            history_weight.append((decay_rate)**(i-1))
    ...
    def forward(self, history_data_list, record_len):
        assert len(history_data_list) == self.frame
        ...
        confidence_mask = []
        ones_mask = torch.ones_like(confidence_score[0]).to(confidence_score[0].device)
        zeros_mask = torch.zeros_like(confidence_score[0]).to(confidence_score[0].device)
