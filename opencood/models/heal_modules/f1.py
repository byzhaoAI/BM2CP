...
from opencood.models.heal_modules.feature_alignment import AlignNet
...
class CoVQMF1(nn.Module):   
    def __init__(self, args):
        ...
        self.heal_conv = nn.Conv2d(modal_dim*2, modal_dim, 1,1,0)

        # setup backbone (very light-weight)
        setattr(self, f"backbone", ResNetBEVBackbone(args['backbone_args']))
        # if sensor_name == "camera":
        #     camera_mask_args = model_setting['camera_mask_args']
        #     setattr(self, f"crop_ratio_W_{modality_name}", (self.cav_range[3]) / (camera_mask_args['grid_conf']['xbound'][1]))
        #     setattr(self, f"crop_ratio_H_{modality_name}", (self.cav_range[4]) / (camera_mask_args['grid_conf']['ybound'][1]))
        setattr(self, f"aligner", AlignNet(args['aligner_args']))
    ...
    def forward(self, batch_dict):
        ...
        if self.use_lidar and self.use_camera:
            feature = self.heal_conv(torch.cat([lidar_feature, x], dim=1))
        if not self.use_camera:
            feature = lidar_feature
        else:
            feature = x
        
        feature = eval(f"self.backbone")({"spatial_features": feature})['spatial_features_2d']
        feature = eval(f"self.aligner")(feature)
        return feature
