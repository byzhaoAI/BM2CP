...
from opencood.models.fmcp_v6_modules.base_bev_backbone_resnet import ResNetBEVBackbone
from opencood.models.fmcp_v6_modules.attentioncomm import AttenComm
from opencood.models.fmcp_v6_modules.sensor_blocks import ImgCamEncode
...
class MultiModalFusion(nn.Module):
    ...
    def forward(self, feats, modality_adapter):
        ...
        """
        # fused_feat = torch.sum(torch.stack(feats), dim=0)
        fused_feat = self.adapt_conv(torch.cat(feats, dim=1))
        return self.relu(fused_feat)
        """
        return fused_feat
    
class PointPillarFMCPV6V2V4Real(nn.Module):
    ...
    def __init__(self, args):
        super(PointPillarFMCPV6V2V4Real, self).__init__()
        # cuda选择
        ...
        self.modality_adapter = nn.Embedding(modality_args['num_modality'], pc_args['point_pillar_scatter']['num_features'])
        self.fusion = MultiModalFusion(modality_args['num_modality'], pc_args['point_pillar_scatter']['num_features'])
    ...
    def forward(self, data_dict):   # loss: 5.91->0.76
        ...
        if 'image_inputs' in data_dict:
            ...
            x = self.fusion([batch_dict['spatial_features']], self.modality_adapter)
